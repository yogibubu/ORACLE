...
def build_parser(
    *,
    ...
    babel = sub.add_parser("babel", help="Compatibility alias for LINK preprocessing")
    ...
    fortran_audit = gicforge_sub.add_parser(
        "fortran-audit",
        help="Compare MATRIX/NEO GIC/B rows against the vendored Merlino Fortran backend",
    )
...
def main(
    ...
    *,
    ...
    if _is_neo_command(args) and args.gicforge_command == "plan":
        from matrix_neo import write_gicforge_plan_sections
    ...
    if _is_neo_command(args) and args.gicforge_command == "report":
        from matrix_neo import gic_report_from_xyzin, write_gic_report
    ...
    if _is_neo_command(args) and args.gicforge_command == "gaussian-input":
        from matrix_neo import write_gicforge_gaussian_input
