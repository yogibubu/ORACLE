...
coordinates.  ORACLE stores the linear \texttt{RPck} components in the frozen
GIC definition because their B rows are analytic linear combinations of
dihedral B rows.  The Gaussian block writes those \texttt{RPck} components as
inactive auxiliaries and derives the selected active \texttt{QPck} and
\texttt{PhiP} functionals from consecutive \texttt{RPck} pairs, matching
Merlino3.0 \texttt{PrtPckQP}.
...
endocyclic torsions; \\
\texttt{RPck} & inactive Gaussian auxiliary and frozen ORACLE linear
ring-puckering component with analytic B row; \\
...
frozen ORACLE B matrix uses these linear components directly.  For Gaussian
ReadGIC export, consecutive selected \texttt{RPck} pairs are written as
inactive auxiliaries and converted to active puckering coordinates
\texttt{QPck} and \texttt{PhiP}:
\[
...
is endocyclic torsional, but the Gaussian-active coordinates are the independent
\(q\)- and \(\phi\)-like ring deformation coordinates derived from selected
\texttt{RPck} pairs.  If an odd component remains after pairing, it stays as an
inactive \texttt{RPck} auxiliary, as in Merlino3.0.  Fused rings add
\texttt{BtFl} butterfly coordinates around shared bonds.  Butterfly coordinates
remain in the torsional family for pruning because their B rows describe
ring-puckering motion.
...
linear analytic B-matrix evaluation; the Gaussian block marks them inactive and
adds active \texttt{QPck}/\texttt{PhiP} functionals for complete pairs. \\
...
\texttt{gicforge\_report.txt} & ORACLE-native report generated from frozen
