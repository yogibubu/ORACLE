...
\title{\textbf{NEO / GICForge Manual}\\
\large Non-redundant generalized internal coordinates and symmetry-adapted coordinate bases in MATRIX}
...
\begin{abstract}
NEO, currently exposed through the compatible \program{} command name, is the
\oracle{} geometry backend that converts Cartesian molecular
...
NEO/GICForge has one responsibility: starting from a Cartesian molecular
...
machine-readable \oracle{} schema, evaluated analytically on later Cartesian
...
the full periodic table through the shared \oracle{} periodic-table utilities.
...
\noindent
The build writes \texttt{engines/fortran/gicforge/build/gicforge\_legacy}.

The recommended command-line entry point is the shared \oracle{} CLI:

\begin{lstlisting}[style=oracle]
python -m matrix gic --workdir work/gicforge
python -m matrix gic-define geometry.xyz --workdir work/gicdefine
python -m matrix gic-bmatrix oracle.gic.definition.json geometry.xyz
python -m matrix gic-contract --geometry geometry.xyz --workdir work/contract
\end{lstlisting}
...
butterfly coordinates can be introduced.  Spiro systems, rings sharing one
bond, and larger fused systems therefore differ only in the graph relation
between the elementary rings: each ring contributes its own protected
endocyclic bending and puckering source space, while shared bonds add protected
butterfly coordinates.  Ring-puckering components are not blind Fourier sums.
For each ring, NEO compares the bond orders of all endocyclic central bonds
that define the puckering dihedrals.  If they are equivalent within the default
50 percent relative contrast tolerance, even when all are aromatic or partially multiple, the Merlino/Fourier
coefficients are kept.  If some bonds are more rigid than the others, the
dihedrals centered on those bonds receive smaller flexibility factors
\(\sqrt{BO_\mathrm{min}/BO_i}\) before the component is normalized.  The frozen
coordinate therefore carries relative ring rigidity, not a hard simple/double
bond classification.
...
\begin{enumerate}
  ...
  \item endocyclic puckering components \texttt{RPck} for each ring;
...
\subsection{Optional local X--H stretch policy}

By default, X--H stretches are ordinary stretches and therefore enter the same
symmetry-adaptation path as all other bonded stretches.  This is the conservative
Merlino-compatible mode and is recorded as
\texttt{XH\_STRETCH\_POLICY SYMMETRIZE} in the frozen \texttt{\#GIC} section.

For local-mode anharmonic workflows, selected X--H stretches may instead be kept
unsymmetrized.  NEO then tags them as a separate
\texttt{LOCAL\_XH\_STRETCH} family.  The coordinates remain analytic
\texttt{R(i,j)} stretches and participate in rank checking as stretch rows, but
the symmetry projector does not combine them into SALCs.  This keeps the local
oscillators available to GF, VSCF or DVR while leaving the rest of the GIC basis
unchanged.

The selection can be made globally with \texttt{--xh-stretch-policy local-all},
by one-based bond selectors such as \texttt{--local-xh-bond 4-17}, or by local
geminal class with \texttt{--local-xh-class XH}, \texttt{XH2} or
\texttt{XH3}.  The class is assigned from the number of hydrogens bound to the
same heavy atom.  \texttt{XH2} and \texttt{XH3} groups are useful when geminal
couplings are expected to be stronger than couplings to the rest of the
molecule.  If a bond or class selector is given without an explicit policy, the
CLI uses \texttt{local-selected}.  The same fields are written to
\texttt{\#GIC} as \texttt{LOCAL\_XH\_BONDS} and
\texttt{LOCAL\_XH\_CLASSES}, so a later build can reproduce the choice without
...
which \program{} builds protected ring-puckering components.  ORACLE stores the
linear \texttt{RPck} components in the frozen GIC definition because their B
rows are analytic linear combinations of dihedral B rows.  The Gaussian block
exports the selected \texttt{RPck} components directly.  The polar
\(q,\phi\)-like descriptors are derived in post-processing from those
components, because \(\Phi\) is a phase in the puckering subspace rather than a
linear symmetry coordinate and is singular at \(Q=0\).  This rule is used for
isolated and fused rings alike.
...
\texttt{QPck} & post-processing ring-puckering amplitude-like descriptor built
from \texttt{RPck} components; \\
\texttt{PhiP} & post-processing ring-puckering phase-like descriptor built from
\texttt{RPck} components; \\
\texttt{RPck} & frozen ORACLE linear ring-puckering component with analytic B
row and Gaussian ReadGIC export; \\
...
\]
The primitive torsions are first transformed to \texttt{RPck} components.  The
frozen ORACLE B matrix and Gaussian ReadGIC export use these linear components
directly.  Consecutive selected \texttt{RPck} pairs can be converted after the
run to puckering descriptors:
\[
  Q = \sqrt{RPck_i^2 + RPck_j^2}, \qquad
  \Phi = \operatorname{atan2}(RPck_j,RPck_i).
\]
...
\noindent
For six-membered and larger isolated rings, the same rule is used: the
primitive basis is endocyclic torsional, the optimized Gaussian coordinates are
the selected \texttt{RPck} components, and the \(q,\phi\)-like descriptors are
computed later.  Fused rings are treated consistently: each elementary
fused-ring cycle contributes its own protected ring-puckering source space, the
fused set is reduced only within the protected ring family, and the surviving
\texttt{RPck} components are exported directly.
Fused rings also add \texttt{BtFl} butterfly coordinates around shared bonds.
Butterfly coordinates remain in the torsional family for pruning because their
B rows describe ring-puckering motion.

For direct Gaussian \texttt{ReadAllGIC} optimizations, the exported block must
remain a complete non-redundant internal-coordinate basis.  Although
\program{} records the total-symmetric irrep and the list of total-symmetric
GICs in the frozen \xyzin{} contract, Gaussian's Berny optimizer still requires
the full internal basis to build its metric.  The Gaussian input writer
therefore exports all final GICs, but marks every non-totally-symmetric final
coordinate as \texttt{Frozen}.  Only the totally symmetric block remains
variable in a symmetry-preserving Gaussian optimization.  MATRIX-controlled
optimizers and least-squares workflows use the saved total-symmetric list to
select symmetry blocks internally without relying on Gaussian syntax.

\section{Coordinate Families}

\program{} builds primitive coordinates from the saved covalent graph unless a
fragment mode requests otherwise.  Weak complexes have two supported models.
In \texttt{SPECIAL\_COORDINATES} mode, fragments remain separate bodies and
their relative motion is described by protected fragment-center,
fragment-translation and fragment-orientation coordinates.  In
\texttt{PSEUDO\_BONDS} mode, \program{} first tries the Merlino/BDPCS3
X--H\(\cdots\)Y hydrogen-bond detector; if no chemically valid hydrogen bond
connects the relevant fragments, it falls back to the closest inter-fragment
atom pair.  The selected H\(\cdots\)Y or closest contact is added only to the
GIC construction graph.  It is serialized in \xyzin{} under
\texttt{[PSEUDO\_BONDS]} and does not alter the saved covalent topology.
Pseudo-bonds do not generate artificial rings: ring coordinates are built only
from the original covalent ring section.
...
Local X--H stretch & Optional unsymmetrized \texttt{R(X,H)} rows selected by
\texttt{--xh-stretch-policy}, \texttt{--local-xh-bond} or
\texttt{--local-xh-class}.  They remain stretch coordinates but form their own
symmetry block and are kept local for VSCF/DVR workflows. \\
...
Ring torsion & Endocyclic torsions are converted to \texttt{RPck} components
before pruning.  The frozen ORACLE basis keeps selected \texttt{RPck} rows for
linear analytic B-matrix evaluation and Gaussian export.  \texttt{QPck} and
\texttt{PhiP} are derived post-processing descriptors, not active Gaussian
...
Fragment and virtual-center coordinates are treated as a separate protected
class.  These coordinates include fragment-center distances, fragment-center to
atom distances, fragment translations, fragment orientations and atom to
virtual-center distances for ring centers, bond centers and contact centers.
They are not ordinary valence-graph primitives, and they can carry chemical
information that no single stretch, bend or torsion should be allowed to erase.
For this reason \program{} first tests all protected special coordinates
against each other by the same analytic B-matrix modified Gram--Schmidt rank
test used by the Fortran77 routine \texttt{ORCGSEL}.  Only after that protected
pass does it use ordinary primitives to complete the vibrational rank.  A
special coordinate may be skipped only when its own row is singular or linearly
dependent on previously accepted special rows.  If the independent protected
set alone is larger than the vibrational rank, \program{} stops with a contract
error instead of silently deleting a protected coordinate.

For a non-covalent complex this policy gives two deliberately different
coordinate bases for the same geometry.  The fragment model keeps
inter-fragment rigid-body coordinates protected.  The pseudo-bond model removes
fragment coordinates from the selected basis and lets the standard
stretch/bend/torsion/OOP generator operate on the graph augmented by the
selected weak contact.  Primitive candidates are still reduced by analytic
B-matrix rank; the pseudo-bond is an internal-coordinate edge, not a request to
keep every primitive generated around it.

The frozen \xyzin{} schema records this distinction explicitly.  Ordinary
coordinates are written with \texttt{CLASS=ORDINARY}.  Fragment and
virtual-center coordinates are written with the protected class token
\texttt{SPECIAL\_PROTECTED}.  The header also records the rank method
\texttt{analytic\_b\_matrix\_mgs\_greedy}.  The reduction policy is the
protected-first policy described above, serialized in \xyzin{} as
two adjacent tokens:
\texttt{SPECIAL\_PROTECTED\_FIRST\_THEN\_ORDINARY\_} and
\texttt{ANALYTIC\_RANK}.
This makes restarts reproducible and tells downstream programs that the
protected coordinates are part of the molecular model, not disposable
...
coordinates, ring bends, ring torsions, local out-of-plane/improper blocks
around the same center and protected special-coordinate blocks.  Protected
special-coordinate blocks are fragment-center distances, fragment-center to atom
distances, fragment translations, fragment orientations and atom to
virtual-center distances.  There is no global fallback that mixes every
primitive in the molecule.  This preserves the meaning of the final coordinates
and makes the result comparable between Fortran and Python.

Pseudo-bond H-bond coordinates are not placed in protected special blocks.  Once
the pseudo-bond is selected, the resulting stretches, bends and torsions belong
to the ordinary homogeneous valence blocks.  This mirrors the Fortran
\texttt{HBOND}/\texttt{MkHBnd} idea: connect the graph through a weak
H\(\cdots\)Y contact, then use ordinary internal coordinates.
...
Signed coordinates carry the parity of the Cartesian operation.  Torsions,
improper dihedrals, out-of-plane coordinates, butterfly coordinates and
\texttt{RPck} ring-puckering components are pseudoscalar rows: after the atom
permutation and canonical coordinate orientation have been applied, their
primitive transformation is multiplied by \(\det R\).  This prevents planar
out-of-plane or puckering coordinates from being labelled as totally symmetric
under a molecular-plane reflection.  Stretches, valence angles and ring bends
remain scalar coordinates and do not receive this determinant factor.

\noindent
Ring-puckering components require one additional invariant because
\texttt{RPck} coordinates are already linear combinations of redundant
endocyclic dihedrals.  For \texttt{RPck} source spaces of any dimension,
MATRIX can verify the projector in the physical B-row source space used by the
analytic Wilson B matrix.  This is a coordinate-family rule, not a
molecule-specific patch: the symmetry operation must map the selected
puckering-source rows into the same row space that downstream GF, fitting and
optimization modules will differentiate.  When the Merlino-compatible
primitive-space projector already closes the block, the B-row projector is
accepted only if it preserves the same irrep sequence.  If the primitive
projector does not close but a finite independent B-row source representation
does, the B-row representation can be used to avoid an artificial local-SALC
fallback.  If neither representation closes, MATRIX stops or records the
explicit fallback instead of inventing a new coordinate family.
...
\noindent
A second, independent validation is performed by GF/PED when
\texttt{--symmetry-blocks} is requested.  In a correctly symmetrized GIC basis,
the internal \(F\) and \(G\) matrices cannot contain couplings between
different irreducible representations, except for numerical roundoff.  MATRIX
therefore treats such off-block couplings as a contract error to be fixed in
the coordinate/symmetry layer; it does not silently solve the full problem and
label the result as symmetry-blocked.

\section{Frozen Schema and Public API}

The production public API is the \texttt{GICForge} service in
\texttt{oracle\_gic}.  A fresh definition run writes a
\texttt{oracle.gic.definition.v1} schema containing:
...
\texttt{gicforge\_report.txt} & MATRIX-native report generated from frozen
\xyzin{} state with rank method, reduction diagnostics, family counts,
protected-coordinate counts, symmetry source blocks and final GIC labels. \\
...
\texttt{GICSYM}. \\
\texttt{HBOND} & Legacy Merlino keyword for detecting X--H\(\cdots\)Y contacts.
In MATRIX/NEO the corresponding mode is
\texttt{--fragment-mode pseudo-bonds}; detected H-bonds are preferred over the
closest-pair fallback and are written as \texttt{KIND=HBOND}. \\
\texttt{JOINFR} & Legacy Merlino fragment-joining concept.  MATRIX keeps the
saved covalent topology unchanged and records any weak graph-joining contact in
\texttt{\#GIC/[PSEUDO\_BONDS]}. \\
\bottomrule
\end{longtable}

\noindent
The Python CLI exposes the fragment choice explicitly:

\begin{lstlisting}[style=oracle]
matrix neo plan molecule.xyzin --fragment-mode special-coordinates
matrix neo build molecule.xyzin --fragment-mode special-coordinates

matrix neo plan molecule.xyzin --fragment-mode pseudo-bonds
matrix neo build molecule.xyzin --fragment-mode pseudo-bonds
\end{lstlisting}

\noindent
For the formic-acid--water probe used in the regression suite, the first mode
produces protected fragment distance/translation/orientation coordinates with
full B rank, while the second mode detects the O--H\(\cdots\)O contact, writes
\texttt{1 5 6 KIND=HBOND}, and produces only ordinary stretch, bend and torsion
coordinates.
...
\begin{itemize}
  ...
  \item absence of cross-irrep \(F/G\) couplings in symmetry-block GF/PED
  diagnostics;
...
\begin{lstlisting}[style=oracle]
python -m matrix gic-contract --geometry examples/pyrene.xyz \
...
\noindent
Both layers must mirror \texttt{oracle\_gicforge.policy}: primitive families,
reduction classes, rank method, protected-first pruning and symmetry source
blocks are shared contract data, not backend-local choices.

\noindent
The readable ORACLE report is generated directly from the frozen \xyzin{}:

\begin{lstlisting}[style=oracle]
matrix gicforge report molecule.xyzin gicforge_report.txt
\end{lstlisting}

\noindent
The Python port in \texttt{oracle\_gic.gicforge\_python} is a faithful porting
...
\begin{itemize}
  ...
  \item spiro systems with two rings sharing one atom, which must keep the
        Merlino \texttt{Spir} inter-ring angle block;
