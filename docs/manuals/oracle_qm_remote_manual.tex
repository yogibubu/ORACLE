...
ORCA calculations can be launched on \host{} for electronic-structure data.
The MATRIX ORCA adapter currently promotes the final Cartesian geometry and,
when ORCA prints a Cartesian Hessian, the shared
\texttt{\#CARTESIAN\_HESSIAN} section used by GF/PED.  Quantities not yet
normalized into \xyzin{} remain external data and must not be parsed privately
...
\texttt{--promote auto} promotes Molpro outputs and ORCA outputs because those
promotion paths are unambiguous.  Gaussian promotion is deliberately explicit
because a single log can contain different useful sections; choose
\texttt{gaussian-log-hessian}, \texttt{gaussian-rovib},
\texttt{gaussian-electronic} or \texttt{gaussian-fchk}.
