...
global equilibrium matrix once and keeps \(G^{-1}\) as a reference metric.  The
DVR plan records
...
DVR.  Along a scan or grid the B matrix, \(G(Q)\), and \(G(Q)^{-1}\) generally
depend on the large-amplitude coordinates \(Q\).  A production DVR must
therefore either recompute the selected metric block at each grid geometry or
use an explicit derivative expansion of \(G(Q)\).  The GF export is the
equilibrium reference used for planning, diagnostics, one-dimensional bootstrap
estimates and consistency checks.
...
uses this table for planning, but production grids must recompute \(G(Q)\) or
include metric derivatives. \\
...
planning at the reference geometry. \\
\texttt{\detokenize{*_large_amplitude_}}\newline
